---
title: "Hospital admissions in the Omicron wave"
params:
  prepared_by:
    label: "Prepared by:"
...
caption_text <- paste0(
         "Richard Martin-Nielsen | Data: Office of Statistics Portal osp.stat.gov.lt | Prepared ",
         Sys.Date())
...
Admissions data is not available for Lithuania before February 2021, so an estimate of the maximum
reached in the December 2020 wave is used.
...
lt_hosp_cv19 <- lt_hosp_data %>%
  ...
  mutate( event = str_to_title(gsub("_"," ", event))) %>%
    ...
    event == "Admissions Oxygen" ~ "with Oxygen",
    event == "Admissions Mv"     ~ "on Ventilator",
...
lt_hosp_nrml %>%
  ...
  ft_theme() +
  scale_color_ft("oxford") +
  theme(legend.position = "none") +
    labs(title="Lithuania - Case, Hospitalisation and Deaths data",
       subtitle="7 day averages - December 2020 wave max = 100*",
       y="",
       x="Date",
       caption=paste0("* Estimate used for maximum admissions because data only available from February 2021\n  ", caption_text)) +
  scale_x_date(limits = c(ymd("2020-12-01",NA)),expand = expansion(add = c(0,100))) +
  scale_y_continuous(limits = c(0,NA))
```

## Hospital admissions since late 2020

This plot shows COVID-19 related hospital admissions since late 2020.
...
lt_hosp_cv19 %>%
  ...
  theme(legend.position = "none") +
    labs(title="Lithuania - Hospitalisation admissions",
