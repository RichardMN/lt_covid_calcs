...
The dataset only considers first infections - individuals who are infected
more than once are not counted again. With more reinfections, these data
now diverge from the national data considerably.
...
```{r cases_graph_contributions_adults, message=FALSE, warning=FALSE}
...
```{r cases_graph_total_fractions_adults, message=FALSE, warning=FALSE}
infection_comparisons_adults %>%
  group_by(event) %>%
  mutate(pc_cases_7d_mean = zoo::rollmean(count,k=7, fill=NA, align="center") ) %>%
  ungroup() %>%
...
```{r vaccine_effectiveness_7d_adults, fig.height=6, fig.width=6, include=FALSE}
# 
