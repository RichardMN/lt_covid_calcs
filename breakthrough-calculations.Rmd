...
For each day, the mean days since vaccination for all those fall ill having
had two doses of vaccine.^[This is encoded as `r2i2_mdsv`] This value varies
...
Lithuanian figures almost all fall between the UK's vaccine-specific values
for the two vaccines
in use, reflecting the mix of vaccines in use in the Lithuanian population (and that this
mix is masked by aggregation of the open data available). That the figures for the 20+ weeks
cohort are below the UK's Vaxzevria estimate may reflect that Lithuania used 
more Vaxzevria earlier in 2021, meaning these vaccinations make up a much larger proportion
of individuals with a larger mean days since vaccination figure, and Lithuania did
not extend the spacing between doses of Vaxzevria.
...
|second character of the character pair (letter)||
|------|-------|
...
![Diagram from the OSP, annotated in English](open_vakc_nevakc_atvejai_en.png)
...
![From OSP](open_vakc_nevakc_atvejai.png)
